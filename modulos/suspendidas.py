...
import urllib.parse
from .comun import leer_archivo, normalizar_columnas, analizar_crm, separar_telefonos
...
def suspendidas():
    ...
    try:
        ...
        if len(data) == 0:
            st.warning("El archivo no tiene registros.")
            return

        req = [
            "CLIENTE_NRO", "FECHA_REPORTE", "VENDEDOR_EH",
            "VENDEDOR_NOMBRE", "TIPO_VENTA", "CLIENTE_TELEFONO1",
            "NODO_NOMBRE", "CRM_OBSERVACION"
        ]
        ...
        resumen = (
            data.groupby(["VENDEDOR_EH", "VENDEDOR_NOMBRE", "SOCIO_DISPLAY"])
        ...
        resumen["MOSTRAR"] = (
            ...
            resumen["Casos"].astype(str) + " casos)"
        )
        ...
        nombre = str(detalle["VENDEDOR_NOMBRE"].iloc[0])
        ...
        for _, row in detalle.iterrows():
            titular, ref1, ref2 = separar_telefonos(row["CLIENTE_TELEFONO1"])
        ...
        st.text_area(
            ...
            value=texto,
            height=520,
            key=f"txt_susp_{eh}_{len(detalle)}"
        )
        ...
        st.dataframe(
            detalle[[
                "CLIENTE_NRO", "FECHA_REPORTE", "VENDEDOR_EH",
                "VENDEDOR_NOMBRE", "TIPO_VENTA", "CLIENTE_TELEFONO1",
                "NODO_NOMBRE", "CRM_OBSERVACION", "OBSERVACION_SUGERIDA"
            ]],
