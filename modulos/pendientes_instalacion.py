...
def pendientes_instalacion():
    ...
    try:
        ...
        data["SOCIO"] = data.apply(
            lambda x: str(x["VENDEDOR_NOMBRE"]).strip()
            if str(x["VENDEDOR_NOMBRE"]).strip() not in ["", "nan", "None"]
            else SOCIOS_DICT.get(str(x["VENDEDOR_EH"]), "SIN NOMBRE"),
            axis=1
        )
        ...
        data["TEL_TITULAR"] = tels[0]
        data["TEL_REF1"] = tels[1]
        data["TEL_REF2"] = tels[2]
        ...
        vista_cols = [
            c for c in [
                "CLIENTE_NRO", "FECHA_REPORTE", "VENDEDOR_EH", "SOCIO",
                "NODO_NOMBRE", "CRM_MOTIVO", "ANALISIS_CRM",
                "TEL_TITULAR", "TEL_REF1", "TEL_REF2"
        ...
        for _, row in detalle.iterrows():
            texto += (
                ...
                f"📍 Nodo: {row.get('NODO_NOMBRE', '')}\n"
                f"📞 Titular: {row.get('TEL_TITULAR', '')}\n"
            )

            if row.get("TEL_REF1", ""):
                texto += f"📞 Ref. 1: {row.get('TEL_REF1', '')}\n"

            if row.get("TEL_REF2", ""):
                texto += f"📞 Ref. 2: {row.get('TEL_REF2', '')}\n"
        ...
        st.text_area(
            ...
            value=texto,
            height=520,
            key=f"txt_pend_inst_{eh}_{len(detalle)}"
        )
