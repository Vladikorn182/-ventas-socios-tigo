...
def mensaje_whatsapp_general(resumen: pd.DataFrame, detalle: pd.DataFrame, fecha_consulta: date) -> str:
    """Mensaje GLOBAL para enviar al grupo de socios.

    Incluye el resumen y el detalle agrupado por cada socio, para evitar
    enviar mensajes individuales uno por uno.
    """
    ...
    # Mantiene el orden del resumen: socios con más instalaciones primero.
    for _, r_res in resumen.iterrows():
        eh = str(r_res["EH"])
        socio = str(r_res["Socio"])
        total_socio = int(r_res["Instalaciones"])
        detalle_socio = detalle[detalle["EH"].astype(str) == eh].copy()

        lineas += [
            "",
            f"👤 *{socio}*",
            f"🆔 EH: *{eh}* | 🔧 *{total_socio}* inst.",
        ]

        for i, (_, row) in enumerate(detalle_socio.iterrows(), start=1):
            codigo = row.get("Código cliente", "") or "S/D"
            nodo = row.get("Nodo", "") or "S/D"
            turno = row.get("Turno", "") or "S/D"
...
def mostrar_agenda_tecnica():
    ...
    st.subheader("Mensaje global para WhatsApp")
    texto_general = mensaje_whatsapp_general(resumen, detalle, fecha_consulta)
    st.text_area("Copiar reporte global para el grupo", texto_general, height=420, key="mensaje_general_agenda_v3")
    st.markdown(f"[Enviar reporte global por WhatsApp]({link_whatsapp(texto_general)})")

    with st.expander("Opcional: mensaje individual por socio"):
        if detalle.empty:
            st.info("No hay detalle por socio para la fecha seleccionada.")
        else:
            opciones = [f"{eh} - {SOCIOS_EH[eh]}" for eh in sorted(detalle["EH"].unique())]
            seleccion = st.selectbox("Selecciona socio", opciones, key="agenda_socio_select_v3")
            ...
            st.text_area("Mensaje individual", texto_socio, height=220, key="mensaje_socio_agenda_v3")
